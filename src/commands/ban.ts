...
export const execute: CommandExecute = async ({ message, args, locale }) => {
  ...
  if (member?.user.id === message.client.user?.id || WHITELIST.includes(target.id)) {
  ...
  if (member?.user.id === message.author.id) {
    message.channel.send(interpolate(get("GENERIC_ERROR", locale), { code: ERROR_CODES.SELF_IS_MODERATION_TARGET, message: "Self is moderation target" }))
    return false;
  }
  if (member)
    if (
