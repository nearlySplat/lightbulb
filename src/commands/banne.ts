...
export const execute = ({ message, args }: Context): boolean | Promise<boolean> => {
  ...
  const target = message.guild?.members.cache.find(v => v.user.username.toLowerCase() == args.join(" ").toLowerCase() || v.user.id == args[0] || v.user.tag.toLowerCase() === args.join(" ").toLowerCase() || v.displayName.toLowerCase() == args.join(" ").toLowerCase()) ?? message.member;
